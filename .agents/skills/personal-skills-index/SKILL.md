...
| [`manage-crons/SKILL.md`](../manage-crons/SKILL.md) | Review and edit the scheduling-assistant's cron job registry (`crontab.json`), then sync to system crontab. Only invoke as the `scheduling-assistant` persona. |
...
| [`process-inbox/SKILL.md`](../process-inbox/SKILL.md) | Process pending scheduling requests from the scheduling-assistant inbox. Parses markdown+frontmatter `.md` files, applies `schedule`/`cancel` changes to `crontab.json`, syncs to system crontab, then deletes each processed file. |
...
| [`schedule-request/SKILL.md`](../schedule-request/SKILL.md) | Submit a scheduling request to the scheduling-assistant's inbox. Any persona can invoke this to add or cancel a recurring agent task — without directly touching `crontab.json`. |
